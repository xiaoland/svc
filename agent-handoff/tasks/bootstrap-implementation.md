...
- **Objective**: Build and pin the smallest trustworthy Issue-only bootstrap Wrapper that can self-host the real GitHub Issue-to-Draft-PR implementation of associated-PR routing and mixed-surface publication, then prove the candidate through fresh-install end-to-end black-box acceptance.
- **Guardrails**: Keep every product file inside `agent-handoff/` and outside the parent SVC workspace configuration. Do not name the product in code. Wrapper remains transport-only and never creates/interprets PRs, manages worktrees, or persists Agent thread data. GitHub and app-server remain their own authorities. Bootstrap `B`, the Human primary worktree, and candidate `C` use distinct worktrees, branches, runtime directories, and process identities. Only one process may own a binding. The future source Issue must visibly tell the Agent to use only its provisioned candidate worktree and never modify the bootstrap or primary worktree. No GitHub App, webhook, Issue, Cloudflare resource, user-scope instruction, or product code is created until a new explicit start authorizes that exact mutation scope.
...
- **Current Truth**: The work has an isolated remote branch, `feat/github-collaboration-bootstrap`, based at `fc7cbc3c0ba647d41b05e9d5e9bd17977042544c`; its initial commit contains only the isolated SVC markers and planning task packets. The local host has Python 3.12.10, PDM 2.28.0, Node 26.3.0, Wrangler 4.119.0, `gh`, and Codex app-server 0.147.0-alpha.6.5. Wrangler exposes a free `tunnel quick-start` command. Codex app-server exposes `thread/start`, `thread/resume`, `turn/start`, `turn/steer`, `turn/interrupt`, typed turn lifecycle, and lossy persisted turn items; Wrapper therefore mirrors live protocol items but never treats provider resume as a complete tool transcript. The recommended bootstrap is an independent Python 3.12 project with a small async HTTP/JSON-RPC runtime, SQLite transport state, a dedicated GitHub App, Quick Tunnel ingress, and explicit public CLI/health controls. Product implementation has not started.
- **Next Step**: Review and explicitly start bootstrap implementation. The first executing slice is a real app-server protocol probe plus isolated child project skeleton; external GitHub/Cloudflare provisioning remains a later, separately announced mutation within the approved plan.
...
- `operator_cli`: serve, bind, unbind/pause, status, provider probe, tunnel start/stop, and black-box evidence entry points. It exposes settled operational state without leaking secrets or thread content.
...
5. **Bind the real provider without owning it**
   - `bind` validates one active binding per Issue and accepts a pre-provisioned, dedicated Agent worktree as the provider cwd.
   - Start or resume exactly one opaque thread and inject only dynamic Issue/repository/Wrapper addresses plus the durable user-scope collaboration contract.
...
This is a normal Human message and visible safety constraint, not a Wrapper command. The same invariant also belongs in the durable user-scope collaboration instructions, while exact branch/worktree identities remain dynamic Issue context.
