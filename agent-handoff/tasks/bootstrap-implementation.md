...
- **Current Truth**: The isolated bootstrap implementation has been committed and pushed at `9992a7b`; collaboration rules are project-scoped and user-scope instructions remain untouched. Dedicated Wrapper and Agent GitHub Apps are installed only on `xiaoland/svc`, real Issue `#20` is bound to one opaque provider thread, and `feat/issue-20-provider-reconnect` has its own clean candidate worktree at the pinned bootstrap baseline. A free Cloudflare Quick Tunnel using HTTP/2 reaches the loopback ingress; a repository webhook with the six approved events passed a real GitHub ping with HTTP 200. GitHub's settings form did not persist a newly enabled App hook and `/app/hook/config` remained 404, so the current campaign uses the repository webhook for ingress while the Wrapper App remains the read/write identity. The first real quiet-settled turn correctly read the Issue, preserved readiness, created a live sharded mirror, and made no code or PR mutation. It also exposed a deterministic throughput defect: once a turn required six comment shards, a publication exceeded the five-second interval and the pre-publication deadline made every next provider delta immediately overdue, producing GitHub-edit backpressure. A hotfix now schedules the next mirror deadline from publication completion; its slow-publication regression and the full child suite pass. The provider turn is still active, so this is operational evidence rather than completed acceptance.
- **Next Step**: Let the current turn reach an explicit provider terminal event, then deploy the verified mirror-deadline hotfix without touching the candidate or primary worktree. Continue the real Issue discussion until the Agent independently judges readiness, implements only in its candidate worktree, creates a native-linked Draft PR, and completes PR comment/review routing. Record the App-hook absence, direct-cloudflared HTTP/2 requirement, health `active_turn` mismatch, sharding noise, and provider/mirror throughput as black-box findings; no local test result substitutes for the unfinished Issue-to-Draft-PR campaign.
