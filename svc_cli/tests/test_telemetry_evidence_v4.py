...
def _bundle(material: bytes = b'{"message":"hello"}\n') -> tuple[object, bytes, dict[str, bytes]]:
    material_name = "native/root.jsonl"
    source = SourceRef(material=material_name, line=0, byte_start=0, byte_end=len(material))
...
def test_v4_round_trip_requires_and_binds_trajectory_and_materials(tmp_path: Path) -> None:
...
def test_v4_rejects_trajectory_or_material_tampering(tmp_path: Path) -> None:
    ...
    for member in ("trajectory.jsonl", "native/root.jsonl"):
        ...
        with zipfile.ZipFile(BytesIO(original.getvalue())) as source, zipfile.ZipFile(target, "w") as output:
            for info in source.infolist():
                value = source.read(info.filename)
                output.writestr(info.filename, value + b"x" if info.filename == member else value)
