...
def test_schema_and_cursor_pages_preserve_exact_utf8_and_binary_bytes(tmp_path: Path) -> None:
    ...
    while True:
        ...
        for item in response["items"]:
            ...
            assert item["frame_sha256"] == hashlib.sha256(
                frames[item["native_index"]]
            ).hexdigest()
...
def test_cursor_and_reference_scope_errors_keep_the_request_union_closed(tmp_path: Path) -> None:
