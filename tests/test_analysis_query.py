...
from svc_cli.analysis.query import query_schema
...
def test_schema_method_and_overview_form_one_navigation_contract(tmp_path: Path) -> None:
    schema = query_schema()
...
def test_match_intersection_distinguishes_results_from_coverage_status(tmp_path: Path) -> None:
...
def test_cursor_continuation_is_scoped_and_requests_are_closed(tmp_path: Path) -> None:
    ...
    invalid_cases = (
        (other, {"intent": "match", "cursor": first["next_cursor"]}, "cursor-scope-mismatch"),
