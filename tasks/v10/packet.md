...
- **Current Truth**:
  ...
  - Candidate staging is deletion-aware and allowlisted: it stages Towncrier's removed fragments along with the prepared release outputs, then refuses any unexpected path. `release/svc` reuses only an open candidate PR; a merged historical PR on the same branch is not a mutable candidate.
...
- **Next Step**: Verify deletion-aware candidate staging and open-only PR reuse in an isolated worktree, push the correction to `main`, then complete publication through the reviewed Release PR and protected environment.
